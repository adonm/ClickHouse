...
echo "--- SYSTEM CLEAR ICEBERG MANIFEST PRUNE CACHE ---"
$CLICKHOUSE_CLIENT -q "SYSTEM CLEAR ICEBERG MANIFEST PRUNE CACHE"
echo "OK"

echo "--- ProfileEvents existence ---"
$CLICKHOUSE_CLIENT -q "SELECT event FROM system.events WHERE event LIKE '%CatalogCache%' OR event LIKE '%ManifestPrune%' ORDER BY event"
