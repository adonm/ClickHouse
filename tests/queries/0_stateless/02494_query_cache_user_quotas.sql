...
-- same user, so any concurrent test caching a query result perturbs the expected entry counts.
...
SET query_cache_tag = '02494_query_cache_user_quotas';

SYSTEM CLEAR QUERY CACHE TAG '02494_query_cache_user_quotas';
...
SELECT 'Expect no entries in the query cache', count(*) FROM (SELECT * FROM system.query_cache WHERE tag = '02494_query_cache_user_quotas') AS test_query_cache;

SET query_cache_max_size_in_bytes = DEFAULT;
SELECT 'Run SELECT again but w/o quota', 1 SETTINGS use_query_cache = true;
SELECT 'Expect one entry in the query cache',  count(*) FROM (SELECT * FROM system.query_cache WHERE tag = '02494_query_cache_user_quotas') AS test_query_cache;
...
SELECT 'Expect two entries in the query cache', count(*) FROM (SELECT * FROM system.query_cache WHERE tag = '02494_query_cache_user_quotas') AS test_query_cache;
