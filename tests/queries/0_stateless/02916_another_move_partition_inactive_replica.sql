...
create table shard_0.to_02916 (x UInt32) engine = ReplicatedMergeTree('/clickhouse/tables/to_02916_' || currentDatabase(), '0') order by x settings old_parts_lifetime=1, max_cleanup_delay_period=1, cleanup_delay_period=1, max_parts_to_merge_at_once=2, shared_merge_tree_disable_merges_and_mutations_assignment=1;

create table shard_1.to_02916 (x UInt32) engine = ReplicatedMergeTree('/clickhouse/tables/to_02916_' || currentDatabase(), '1') order by x settings old_parts_lifetime=1, max_cleanup_delay_period=1, cleanup_delay_period=1, max_parts_to_merge_at_once=2;
