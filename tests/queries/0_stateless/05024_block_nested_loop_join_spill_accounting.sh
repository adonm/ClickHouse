...
#
# The events are read from the packets the server sends with the query rather than from
# `system.query_log`, so that the test does not wait for `SYSTEM FLUSH LOGS` - a server-wide flush
# that on a loaded runner takes far longer than the query.
