...
#define APPLY_FOR_BUILTIN_METRICS(M) \
    ...
    M(DataLakeCatalogCacheBytes, "Size of the DataLake catalog cache in bytes") \
    M(DataLakeCatalogCacheFiles, "Number of entries in the DataLake catalog cache") \
    M(IcebergManifestPruneCacheBytes, "Size of the Iceberg manifest prune cache in bytes") \
    M(IcebergManifestPruneCacheFiles, "Number of entries in the Iceberg manifest prune cache") \
