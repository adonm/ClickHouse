...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, version, 1, "The version of the TimeSeries table: it determines the set of the target tables and their structure. The version is stamped automatically when a table is created and cannot be changed afterwards. The default value must always stay equal to 1 - the version of the tables created before this setting was introduced (see TimeSeriesVersion.h)", 0) \
